...
<div>
    ...
    <form action="/profile/edit/update" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-2 gap-6">
            <div>
                ...
                <div class="flex justify-center my-2">
                    <input type="file" name="image" class="file-input dark:border-slate-400 dark:bg-slate-600 dark:text-white file-input-bordered w-full max-w-xs" />
                </div>
                @foreach ($profiles as $profile)
                <div class="mb-4">
                    <h1 class="font-semibold mb-1 dark:text-slate-400">Username</h1>
                    <input class="input input-bordered w-full dark:bg-slate-600 dark:text-white" name="name" type="text" value="{{ $profile->name }}">
                </div>
                <div class="mb-4">
                    <h1 class="font-semibold mb-1 dark:text-slate-400">Email</h1>
                    <input class="input input-bordered w-full dark:bg-slate-600 dark:text-white" name="email" type="email" value="{{ $profile->email }}">
                </div>
                <div class="mb-4">
        ...
        <div class="flex justify-end mb-4 gap-6">
            <a href="/profile" class="btn btn-outline btn-error ">Cancel</a>
            <button type="submit" class="btn text-slate-200 dark:text-slate-800 btn-success">Save Changes</button>
        </div>
    </form>
</div>
@endsection
